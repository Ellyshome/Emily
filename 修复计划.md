...
> 执行者：AI 开发工具
> 编制日期：2026-09-03
> 状态：**已执行**（2026-09-03，除修复 1 待用户决策 + 新发现坑 AstrBot 镜像版本）

## 执行方式

本计划已按"执行顺序与依赖"逐项执行完毕。执行状态与验证结果见文末「执行报告」。
...
| 修复项 | 状态 | 验证结果 |
|--------|------|----------|
| 修复 7 孤儿删除 | ✅ | `git rm` 7 个文件 + 整链；`emily_core.tools` 转为 namespace 包导入正常；容器重启无 ImportError |
| 修复 8 死属性 | ✅ | `_workitem_agent` 删除；CLAUDE.md 指向 langgraph_engine |
| 修复 10 去重 compose | ✅ | `docker-compose-remote.yml` 已删（与 wecom 字节相同） |
| 修复 14 注释清理 | ✅ | 全库 grep `workitem_agent` 零残留（emily-core 内） |
| 修复 2 session_cleanup | ✅ | config `enabled: false` + handler 改只读巡检；另发现：**config 文件无代码读取，DB `scheduler_jobs` 表才是真来源**（且无 cleanup job） |
| 修复 3 假成功 | ✅ | data_sync/webhook 返回 `success=False` |
| 修复 4 voice_entry | ✅ | 全链摘除（registry/project/tools_consistency/register_api/seed_sql/_diag_tools/toolmgr_cases）+ **新增 `REMOVED_TOOLS` 启动停用机制**；DB `voice_entry` 行已 `is_active=f`（实测） |
| 修复 5 rag_logger | ✅ | knowledge_search_tool 成功/失败双路径接入；**实测** `rag_retrieval_logs` 新增 3 条 pgvector 记录 |
| 修复 6 morning_report | ✅ | TODO + 占位文本清理 |
| 修复 9 副本同步 | ✅ | 核心 message.py + `event_id` 字段；删除插件 3 副本 + 清理 `__init__.py` 重导出 |
| 修复 11 脚本注册 | ✅ | 注册 7 个（run_daily_file_parse/ingest_knowledge/snapshot/verify_langgraph_engine/rag_batch_test/pdfread/migrate_file_purpose），registry 加载 39 个通过；gen_paper_tables 属论文产物不注册 |
| 修复 12 `__all__` | ✅ | 13 个 handler 全部导出，容器内导入验证通过 |
| 修复 1 PG 目录 | ✅ **已执行（用户确认后）** | `db_seeds` → `postgres_data` 迁移成功（78M，PG 数据完整，14 users 实测）；napcat/minimal/wecom 三份 compose 统一为 `./emily-data/postgres_data` 相对路径；三份 compose 的 `/app/db_seeds:ro` 无消费者挂载全部删除；wecom.yml 原来错挂 `./pgdata`（陈旧副本，PG_VERSION 在 `pgdata/emily/` 子目录）也已统一 |
| 新发现 | ✅ **已修复（用户确认后）** | **AstrBot 镜像 4.25.2 → latest=4.27.5 升级完成**，插件 emily_agent 1.1.0 载入成功（`Plugin emily_agent (1.1.0) ... 双通道`），SSE 已连接 emily-core。企微适配器报 40001 invalid credential 是企微凭据过期的既有配置问题（与本次修复无关） |

**端到端验证**：emily-core 重启正常（`_ensure_tool_registry_seed: deactivated 1 removed tools`）；`check_all` 0 error；emy-test 冒烟（"你好"）与知识检索均正常回复；`rag_retrieval_logs` 有真实写入。
