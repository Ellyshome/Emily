...
name: req-verify
description: >
  Emily 验证测试 —— SDD 流水线的 Verify 阶段。根据模块 PRD（规格）与实施计划，化身资深测试工程师对
  Emily 新开发模块/系统/脚本进行专业验证测试：不止判"功能对不对"，更判"规格是否被满足"。
  覆盖测试计划设计、环境准备（Docker状态检查、DB预埋数据、文件系统预设）、测试执行（emy-test IM对话模拟、
  直接API调用、数据库验证、Docker日志检查、LLM流量抓包分析、Session归档分析）、规格符合度判定、
  测试报告生成、临时产物清理。每条用例回指 US-x，产出"规格符合度"与"未覆盖规格"清单。
  产出须遵守 .claude/skills/_shared/constitution.md。
  触发：/req-verify、/验证测试、"帮我测试这个模块"、"验证一下这个功能"、"test this module"。
...
> **最高约束**：本 skill 的产出必须符合 [项目宪法](../_shared/constitution.md)。尤其 §1.3（追溯闭合）、§2（架构铁律 C0~C11）、§3（质量红线 Q1~Q6）。测试报告须含"宪法合规"判定。
...
| 9 | **宪法合规** | 测试中同步核对宪法 §2 铁律（C0~C11）与 §3 红线（Q1~Q6）是否被违反，在报告中单列"宪法合规"判定 |
| 10 | **接线闭合** | 不以"功能跑通"代替"接线闭合"。任何产出物必须有消费者、任何开关必须有读取方；用**静态可达性扫描**（定义 vs 使用）实测，仅定义无使用即 FAIL（宪法 §3 Q6） |
...
- 用户提供了模块目录（通常位于 `Issues/{模块名}/`），需要对其进行验证测试
...
1. 根据用户提供的路径确定模块目录。如果用户只给模块名，用 Glob 在 `Issues/` 下搜索：
   ```
   Glob: Issues/{模块名}/**/*.md
   ```
...
### ⚠️ Step 2.6：接线静态扫描（必做，秒级，宪法 Q6）

**在跑任何运行时测试之前，先做一轮静态可达性扫描**——多数"断线"（死开关 / 零调用 / 零写入）不需要把服务跑起来就能秒级证伪。**跳过本步骤的报告不得判"通过"**。

```bash
# 一键扫描：Config 字段的「定义 vs 使用」对照（高精度）
uv run python scripts/wiring_scan.py --markdown
# → 命中"仅定义无使用（死开关）"时退出码非 0
# 追加 DB 列扫描（低置信度、需人工复核）：加 --with-models
```
...
grep -rn "<新增配置项名>" --include=*.py .    # 仅 1 处命中（定义处）= 死开关
```

| 检查对象 | 判据 | 结论 |
|---------|------|------|
| 新增配置项 | 全仓引用 < 2（仅定义） | ❌ FAIL 未接线 |
| 新增公开方法 / 函数 | 零调用者 | ❌ FAIL 孤岛 |
| 新增 DB 列 | 零写入方 或 零读取方 | ❌ FAIL 半截 |
| 新增注册项（工具/节点/Hook/SOP/Script） | 已注册但不在任何可达调用路径 | ❌ FAIL 孤儿（C11） |

**扫描结果必须原样进入报告"九、接线与可达性"章**。
...
| 维度 | 覆盖内容 | 验证方式 |
|------|---------|---------|
| **接线/可达性（静态）** | **新增/触碰的配置项、公开方法、DB 列、注册项是否有消费方**；定义 vs 使用对照 | 静态扫描：`scripts/wiring_scan.py` + `grep` 定义处 vs 使用处计数（宪法 Q6） |
...
| 类型 | 适用对象 | 要求 | 判定 |
|------|---------|------|------|
| **静态可达性** | 新增/触碰的配置项、公开方法、DB 列、注册项 | 定义 vs 使用对照 | 仅定义无使用 = FAIL |
| **端到端闭环** | 任何"写入类"US | 写入 → **新会话 / 新进程** → 消费方可见 | 消费方读到内容 = PASS；写成功但读不到 = FAIL |
| **跨进程 / 跨重启** | 涉及 interrupt / 缓存 / 内存态的功能 | 运行到挂起 → 重启进程 → 续跑 / 复读 | 重启后仍正确 = PASS |
...
**报告模板**——必须包含以下 11 个章节（见下方第 5 节；在原有 8 章基础上**新增"规格符合度""约束与合规""接线与可达性"三章**）。
...
> 对照 [项目宪法](../_shared/constitution.md)：架构铁律（C0~C11）、质量红线（Q1~Q6）、以及 PRD §4.4 的约束型技术决策是否被遵守。
...
> 宪法 §3 Q6：任何产出物必须有消费者，任何开关必须有读取方。本章用**静态证据**（而非运行时行为）判定——"功能跑通"不能替代"接线闭合"。

### 9.1 静态可达性扫描结果

| 对象 | 定义处 | 使用处计数 | 判定 | 证据 / 命令 |
|------|-------|-----------|------|------------|
| `{配置项}` | `config.py:NNN` | {N} | ✅ / ❌ 仅定义 | `grep -rn "..."` |
| `{公开方法}` | `xxx.py:NNN` | {N} | ✅ / ❌ 零调用 | `grep -rn "..."` |
| `{DB 列}` | `models.py:NNN` | 读 {N} / 写 {N} | ✅ / ❌ 半截 | `grep -rn "..."` |

**扫描命令**：`uv run python scripts/wiring_scan.py --markdown`
**扫描结论**：{无断线 / 发现 N 处断线，详见上表}
...
| **18** | **不做约束与合规判定** | **报告中单列"约束与合规"章：逐条核对架构铁律（C0~C11）、质量红线（Q1~Q6）、以及 PRD §4.4 的约束型技术决策是否被遵守** |
| **19** | **凭"计划说遵守了"就放过 PRD 约束** | **必须实测（依赖清单比对 / 代码检索 / 配置核对），不能只抄计划的"约束落实"表当结论** |
| **20** | **默认值巧合正确就判开关有效** | 开关有效性看**有无读取方**（静态扫描），不看行为是否恰好正确（宪法 Q6） |
| **21** | **写入类需求只测"写成功"** | 必须补"写入 → 新会话/新进程 → 消费方可见"的端到端闭环用例 |
| **22** | **涉及内存态/断点的功能不测重启** | interrupt / 缓存 / 内存态功能必须有"重启后仍正确"用例 |
| **23** | **只测本次模块、不扫存量断线** | 对本次触碰的文件/配置做一次全仓定义-使用扫描，防止历史断线藏身 |
...
| 结论 | 判定条件 |
|------|---------|
| ✅ **通过** | **规格符合度 100%（全部 US 满足）**，所有测试用例 PASS（SKIP 项 < 总数 10% 且有合理原因），无 🔴高 严重度 Bug，架构铁律与 **PRD 约束**均无违反，且**接线闭合无断线（宪法 Q6）** |
| ⚠️ **有条件通过** | 核心路径 US 满足，但存在 US 部分满足/未覆盖，或存在 🟡中 / 🟢低 严重度 Bug，或有 > 10% 用例因环境限制 SKIP，或存在**半截交付（生产侧已合入、消费侧未接）且已登记** |
| ❌ **未通过** | 存在 US 不满足，或**违反 PRD §4.4 约束型技术决策**，或核心路径 FAIL，或存在 🔴高 严重度 Bug，或违反架构铁律，或**存在宪法 Q6 断线（仅定义无使用 / 仅写无读 / 仅注册无调用）**，或 > 30% 用例 FAIL/SKIP |
...
| **接线扫描** | `uv run python scripts/wiring_scan.py --markdown` | 静态可达性：config 字段 / DB 列 定义-使用对照（宪法 Q6） |
...
- [ ] 报告包含所有 11 个章节（环境→计划→规格符合度→结果→Bug→DB验证→运行时可观测性→约束与合规→接线与可达性→结论→附录）
- [ ] **已完成约束与合规判定**：架构铁律（C0~C11）、质量红线（Q1~Q6）、**PRD §4.4 约束型技术决策**逐条核对
...
- [ ] 已执行接线静态扫描（`scripts/wiring_scan.py`），结果进入报告"接线与可达性"章
- [ ] 写入类 US 有"端到端闭环"用例（写入 → 新会话/新进程 → 消费方可见）
- [ ] 涉及 interrupt / 缓存 / 内存态的功能有"跨进程 / 跨重启"用例
- [ ] 无宪法 Q6 断线（仅定义无使用 / 仅写无读 / 仅注册无调用）
