...
| **表数量** | 41 表（含全局状态机 7 表 `sm_*` + 运维模块 5 表 `ops_*`） |
...
| 22 | `sop_checkpoints` | 19 | SOP 执行状态快照（确认流程持久化与恢复） |
...
| 26 | `plan_task_templates` | 18 | 计划任务模板（周期规则/验收标准/工作流定义） |
| 27 | `plan_task_instances` | 27 | 计划任务实例（7 态状态机/周期性/异常/多索引） |
| 28 | `plan_task_logs` | 7 | 计划任务状态变更审计日志（含快照） |
| 29 | `plan_task_deliverables` | 8 | 计划任务交付物（文本/文件/JSON） |
| 30 | `sm_nodes` | 23 | 全景节点主表 — ~90 个标准项目节点（node_id/stage_id/status/precondition_score） |
| 31 | `sm_node_dependencies` | 5 | 节点依赖关系表（from_node_id→to_node_id + weight/required） |
| 32 | `sm_node_deliverables` | 6 | 节点成果物表（node_id→deliverable_name + required/uploaded） |
| 33 | `sm_stages` | 11 | 阶段脉络表 — 7 个阶段进度（total_nodes/completed_nodes/progress/critical_path） |
| 34 | `sm_status_history` | 7 | 状态变更历史表 — 审计用（from_status→to_status/snapshot） |
| 35 | `sm_audit_logs` | 12 | 操作审计日志表（timestamp/operator/action/old_value/new_value/risk_marked） |
| 36 | `sm_simulation_results` | 12 | 推演结果缓存表（阶段四使用，阶段一建表预留） |
| 37 | `ops_tick_log` | 11 | 运维 Tick 执行日志 — 每轮巡检元数据 |
| 38 | `ops_probe_execution` | 7 | 运维探针执行记录 — 每探针每 Tick 一条 |
| 39 | `ops_finding` | 9 | 运维探针发现结果 — 问题记录 |
| 40 | `ops_mail_audit` | 11 | 运维邮箱审计 — mail_uid UNIQUE 幂等去重 |
| 41 | `ops_startup_report` | 22 | 运维冷启动报告 — 组件/业务状态快照 |
...
#### `sm_nodes` — 全景节点主表 (~90 nodes)

| 字段 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|------|------|------|--------|------|------|
| `id` | String | PK | `_new_uuid()` | — | UUID 主键 |
| `node_id` | String(20) | **否** | — | **UNIQUE** | 节点编号如 "1.1" |
| `node_name` | String(500) | **否** | — | — | 节点名称 |
| `stage_id` | Integer | **否** | — | **索引** | 所属阶段 1-7 |
| `parent_section` | String(200) | ✅ | `""` | — | 父级分类 |
| `node_type` | String(50) | ✅ | `"standard"` | **索引** | design/procurement/construction/inspection/survey/license/cost/delivery/standard |
| `status` | String(20) | ✅ | `"NOT_STARTED"` | **索引** | NOT_STARTED/IN_PROGRESS/BLOCKED/DELAYED/COMPLETED |
| `owner` | String | ✅ | `""` | — | 主责部门(人) |
| `approver` | String | ✅ | `""` | — | 审批人 |
| `viewers` | String | ✅ | `"[]"` | JSON | 可见人列表 |
...
| `precondition_score` | Integer | ✅ | `0` | — | 前置满足度 0-100 |
| `block_reason` | String(1000) | ✅ | `""` | — | 阻塞原因 |
| `delay_reason` | String(1000) | ✅ | `""` | — | 延期原因 |
| `risk_level` | String(20) | ✅ | `"正常"` | — | 风险等级 |
| `is_milestone` | Boolean | ✅ | `False` | — | 里程碑节点 |
| `recurrence_type` | String(20) | ✅ | `"SINGLE"` | — | SINGLE/WEEKLY/MONTHLY |
| `recurrence_completed_count` | Integer | ✅ | `0` | — | 周期性完成次数 |
| `sort_order` | Integer | ✅ | `0` | — | 排序 |
| `is_deleted` | Boolean | ✅ | `False` | — | 软删除 |
| `created_at` | String | ✅ | `_utc_now()` | — | — |
| `updated_at` | String | ✅ | `_utc_now()` | onupdate | — |

**索引**：`idx_sm_nodes_stage(stage_id)`, `idx_sm_nodes_status(status)`, `idx_sm_nodes_type(node_type)`

#### `sm_node_dependencies` — 节点依赖关系

| 字段 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|------|------|------|--------|------|------|
| `id` | String | PK | `_new_uuid()` | — | — |
| `from_node_id` | String(20) | **否** | — | 唯一(uq+to_node_id) | 前置节点 |
| `to_node_id` | String(20) | **否** | — | 唯一(uq+from_node_id) | 后置节点 |
| `weight` | String | ✅ | `"1.0"` | — | 依赖权重 |
| `required` | Boolean | ✅ | `True` | — | 是否必需 |
| `created_at` | String | ✅ | `_utc_now()` | — | — |

**唯一约束**：`uq_sm_dep_from_to(from_node_id, to_node_id)`

#### `sm_node_deliverables` — 节点成果物

| 字段 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|------|------|------|--------|------|------|
| `id` | String | PK | `_new_uuid()` | — | — |
| `node_id` | String(20) | **否** | — | 唯一(uq+deliverable_name), **索引** | 关联节点 |
| `deliverable_name` | String(500) | **否** | — | 唯一(uq+node_id) | 成果物名 |
| `required` | Boolean | ✅ | `True` | — | 是否必需 |
| `uploaded` | Boolean | ✅ | `False` | — | 已上传 |
| `file_id` | String | ✅ | `""` | — | 关联文件 |
| `created_at` | String | ✅ | `_utc_now()` | — | — |

**唯一约束**：`uq_sm_dlv_node_name(node_id, deliverable_name)`
...
|------|------|------|--------|------|------|
| `id` | String | PK | `_new_uuid()` | — | — |
| `stage_id` | Integer | **否** | — | **UNIQUE** | 阶段编号 1-7 |
| `stage_name` | String(500) | **否** | — | — | 阶段名称 |
| `status` | String(20) | ✅ | `"未启动"` | — | 未启动/进行中/已完成 |
| `total_nodes` | Integer | ✅ | `0` | — | 总节点数 |
| `completed_nodes` | Integer | ✅ | `0` | — | 已完成节点数 |
| `progress` | Integer | ✅ | `0` | — | 进度 0-100 |
| `milestone` | Boolean | ✅ | `False` | — | 里程碑阶段 |
| `critical_path` | String | ✅ | `"[]"` | JSON | 关键路径节点列表 |
| `boundary_start` | String(1000) | ✅ | `""` | — | 进入条件描述 |
| `boundary_end` | String(1000) | ✅ | `""` | — | 闭环条件描述 |
| `created_at` | String | ✅ | `_utc_now()` | — | — |
| `updated_at` | String | ✅ | `_utc_now()` | onupdate | — |

#### `sm_status_history` — 状态变更历史

| 字段 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|------|------|------|--------|------|------|
| `id` | String | PK | `_new_uuid()` | — | — |
| `node_id` | String(20) | **否** | — | 索引(idx+created_at) | 关联节点 |
| `from_status` | String(20) | ✅ | `""` | — | 变更前状态 |
| `to_status` | String(20) | **否** | — | — | 变更后状态 |
| `operator_id` | String | ✅ | `""` | — | 操作人 |
| `reason` | String(1000) | ✅ | `""` | — | 变更原因 |
| `snapshot` | Text | ✅ | `"{}"` | JSON | 节点完整快照 |
| `created_at` | String | ✅ | `_utc_now()` | — | — |

**索引**：`idx_ssh_node_time(node_id, created_at)`
...
| `target_type` | String(20) | **否** | — | — | node/stage |
| `target_id` | String | **否** | — | — | 目标 ID |
| `action` | String(100) | **否** | — | — | 操作动作 |
| `old_value` | Text | ✅ | `""` | — | 变更前值 |
| `new_value` | Text | ✅ | `""` | — | 变更后值 |
| `reason` | String(1000) | ✅ | `""` | — | 操作原因 |
| `risk_marked` | Boolean | ✅ | `False` | — | 风险标记 |
| `client_ip` | String(50) | ✅ | `""` | — | 客户端 IP |
| `created_at` | String | ✅ | `_utc_now()` | — | — |

#### `sm_simulation_results` — 推演结果缓存（预留）

| 字段 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|------|------|------|--------|------|------|
| `id` | String | PK | `_new_uuid()` | — | — |
| `simulation_name` | String(500) | ✅ | `""` | — | 推演名称 |
| `assumptions` | String | ✅ | `"[]"` | JSON | 假设条件 |
| `affected_nodes` | String | ✅ | `"[]"` | JSON | 受影响节点 |
| `total_days_impact` | Integer | ✅ | `0` | — | 影响总天数 |
| `critical_path_changed` | Boolean | ✅ | `False` | — | 关键路径变更 |
| `risk_level_change` | String(50) | ✅ | `"无变化"` | — | 风险等级变化 |
| `recommendations` | String | ✅ | `"[]"` | JSON | 建议 |
| `new_statuses` | String | ✅ | `"{}"` | JSON | 新状态映射 |
| `created_at` | String | ✅ | `_utc_now()` | — | — |
...
erDiagram
    ...
    plan_task_templates ||--o{ plan_task_instances : "生成"
    plan_task_instances ||--o{ plan_task_logs : "审计"
    plan_task_instances ||--o{ plan_task_deliverables : "交付"
    permission_groups ||--o{ sop_permission_bindings : "绑定"
    sop_business_flows ||--o{ sop_permission_bindings : "绑定"
    users ||--o{ plan_task_instances : "发起/执行"
    sm_nodes ||--o{ sm_node_dependencies : "依赖(from)"
    sm_nodes ||--o{ sm_node_dependencies : "依赖(to)"
    sm_nodes ||--o{ sm_node_deliverables : "成果物"
    sm_nodes ||--o{ sm_status_history : "状态变更"
    sm_nodes }o--|| sm_stages : "所属阶段"
```
...
4. **PostgreSQL advisory lock 是 session 级** —— `PlanTaskScheduler._tick()` 使用 advisory lock 防并发，但锁在 session 关闭时自动释放。使用 `get_session_raw()` 手动管理 session 生命周期。
...
8. **全景节点导入幂等性** — `import_nodes.py` 默认增量模式（跳过已存在的 node_id），使用 `--reset` 清空重建。阶段四推演表 `sm_simulation_results` 预留但未使用。
