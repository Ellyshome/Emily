...
| 模块 | 职责 | 关键文件 | 依赖 |
|------|------|----------|------|
| **SkillRegistry** | 扫描 `skills/` 加载 `.skill.yaml` → Skill 定义缓存 → 热重载；含 `_derive_sop999_tools()` 运行时派生 SOP-999 工具直调白名单 + `_derive_fallback_by_type()` 按 category 分组派生兜底文本 | `skill/registry.py`, `skill/parser.py` | `.skill.yaml` 文件、`tool_registry` 表 |
| **SOP 匹配** | LLM 语义匹配（Registry 仅提供目录，不做匹配） | `session/session_agent.py._recognize_intent()` | LLMClient, SOPIntentRegistry |

**节点域能力（2026-09-23 全景节点需求落地）**：

| 能力 | 触发 | 产出 |
|------|------|------|
| 模板库只读检索（`read_node_template`） | 对话提问 / 控制台模板面板 / 装配链路 | 模板清单与详情（成果清单 / 对象声明 / 附件清单） |
| 按模板装配草稿（`build_node_draft`） | 选模板 + 选项目 | 四类候选集合（参与单位 / 成员 / 共享文件 / 前置成果）+ 每条依据 + 未解析项；**只读不落库** |
| 模板锚定门禁 | 文件归档后（`ArchiveHandlerRegistry`） | 命中 → 装配落临时节点（携带模板来源）；**未命中 → 零建节点，只出「缺模板」提案** |
| 三档归属判定 | 文件归档后 / 对话询问 | 档位结论 + 依据（规则条目 / 模板编号 / 来源文件）；**只出建议、不改图** |
| 文件→完成反推 | 文件归档后 | 「该文件像是 X 节点的成果」提示 + 询问批准；**不自动改状态** |
| 收容与迁正 | 档B/C 落点 / 认领 / 归位 | 两层临时节点 + 未归类收容节点（懒创建、项目唯一）；迁正**编号不变**、全程留痕 |
| 节点停用 / 恢复 | 人工（L4+） | 退出管控（不计完成度 / 不提醒 / 不可挂新任务），非终态可恢复 |

**归档后分析接线**：归档收口（`FileManager.archive_upload` / `FileApplication.handle_file`）只做一处
`asyncio.create_task(ArchiveHandlerRegistry.dispatch(...))`；分析能力经注册表接入，处理器逐个隔离 fail-open，
**不阻断文件归档与人工建节点主链路**。

**10 份 SOP**：SOP-000(编写标准) / SOP-001(会议) / SOP-002(事件) / SOP-003(任务) / SOP-004(文件) / SOP-005(查询) / SOP-007(长期记忆) / SOP-008(待解决) / SOP-011(节点管理) / SOP-999(工具直调兜底——白名单派生 + 写操作确认)
...
#### 5.5.1 文件管理模块 M5 — 用途分类与附件链

##### FilePurpose 五类三层正交分类

文件用途采用 **三层正交分类**：第一层按文件性质分为 5 类，第二层按保密级别约束访问，第三层按项目生命周期阶段限定可见范围。

| 用途枚举 | 中文名 | 含义 | 典型文件 |
|----------|--------|------|----------|
| `EVIDENCE` | 证据文件 | 过程留痕，可追溯、不可篡改 | 验收照片、签证单扫描件、检测报告 |
| `RECORD` | 记录文件 | 业务输出的正式文档（默认） | 会议纪要、任务单、流转单附件 |
| `DESIGN` | 设计文件 | 可迭代的工程图纸/模型/规格书 | CAD 图纸、BIM 模型、技术规格书 |
| `REFERENCE` | 参考资料 | 辅助决策的规范/标准/案例 | 国标 PDF、地方法规、历史项目案例 |
| `CHAT` | 聊天文件 | 群聊/私聊中的临时性文件 | 截图、随手拍照片、非正式传递文件 |

**分类规则引擎** `file_rule_engine.py` 根据文件扩展名 + 上传上下文（消息来源、会话类型）自动推断初始 purpose，同时支持用户手动修正。`purpose_confirmed` 标志位区分自动推断与人工确认。

##### attachment_of 主从附件链模型

`attachment_of` 字段实现 **主从文件链**（master-slave attachment chain）：

```
master_file (主文件，如 CAD 图纸)
  ├── attachment_1 (附件，如设计说明)
  ├── attachment_2 (附件，如材料清单)
  └── attachment_N
```

- **主文件**：`attachment_of IS NULL` —— 独立文件
- **附属文件**：`attachment_of = master_file.id` —— 挂载在主文件下的附件
- **链式操作**：删除主文件时级联解除附属文件关联（`attachment_of` 置 NULL，不物理删除）
- **遍历工具**：`link_to_master` / `unlink_attachment` / `list_attachments` 三个工具操作附件链
...
| **ToolManager** | BusinessFlowToolRegistry 聚合层：统一调用入口、自描述、CLI 测试接口 | `tools/manager.py`, `scripts/toolmgr.py` |
| **ScriptManager** | 脚本聚合层（与 ToolManager 平级）：管理 `scripts/` 目录下全部开发者/维护脚本，统一 check/run/test/export 入口。bootstrap 从清单 `auto_run` 枚举替换硬编码 | `emily_core/scripts/manager.py`, `emily-data/config/scripts_registry.yaml`, `scripts/scriptmgr.py` |
...
| **本地回退** | 零依赖 TF 关键词搜索 .md/.txt（stage/role 过滤，多目录：项目资料 + 公司制度） | `providers/rag/local_fallback.py` |
| **公司制度仓库** | 公司规章制度文件存放目录（管理制度/办法/操作规程），检索时作为知识来源之一 | `emily-data/company_policies/` |
| **session.md** | Emy 核心人格 + 回复规范 + 意图路由（5 层结构：角色定位 / 业务背景三书 / 当前上下文 / 能力清单 / 行为规范与任务指令） | `emily-data/prompts/session.md` |
| **workitem.md** | 执行 Agent 角色 + 规划规则 + 回复合成规则 | `emily-data/prompts/workitem.md` |

### 5.9 计划任务（最新模块）— 已退役

> **⚠ 已退役（2026-09-22 整体移除）**：PlanTask 计划任务模块及其后继「系统调度器模块」（`emily_core/scheduler/`，14 个作业处理器）已整体移除；`plan_task_*` 与 `scheduler_*` 表保留为空表（不做 DROP）。以下为历史记录，仅供参考。
...
用户消息 → SessionAgent
  ...
  │   │
  │   └─ 未命中 / 工具能力请求 (无专属SOP)
  │       → 明确工具能力 → 路由 SOP-999-SYS（工具直调兜底）
  │       → SkillExecutor step-01 LLM 从派生白名单选工具+推参
  │       → step-02 __DYNAMIC__ 直调（写类工具强制确认）
```
