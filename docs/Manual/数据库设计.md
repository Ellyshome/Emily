...
11. **`project_nodes` 节点层级与进度字段（2026-08-12 补齐）** —— `progress`（VARCHAR，0.00-100.00 字符串，由成果完成度汇总）、`parent_node_id`、`child_weight` 三列后加，由 `_ensure_columns` 自动 ALTER 补齐。同时新增 `uq_pn_node_id` UNIQUE 约束（`node_id` 业务主键唯一，配合 `ProjectNodeRepo.create` 幂等检查防重复创建）。`NodeSnapshot` 同步加 `progress` 字段供状态机引擎读取。父子嵌套深度上限 3 层（`MAX_PARENT_DEPTH`），单父子节点上限 100（`MAX_CHILDREN_PER_PARENT`），`mount_child` 做双向循环检测。
