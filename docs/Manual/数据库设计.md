...
14. **创建节点需携带必需成果（2026-09-21）** —— `CreateNodeCommand` 新增 `deliverables` 声明字段；`NodeService.create_node` 强制至少一条 `is_required=true`（否则拒绝，`error_code=40003`），并在同一次调用内落库成果。批量建树不再单独创建成果（原 Phase 2 移除）。
