...
| 4 | `messages` | 23 | 全量通讯记录（入站+出站，含前导消息）+ `source` 流量性质暗记 |
| 5 | `message_attachments` | 10 | 消息附件关联中间表（一消息多附件） |
| 6 | `projects` | 13 | 项目基本条件与生命周期阶段 |
| 7 | `events` | 16 | 项目事件记录 |
| 8 | `tasks` | 14 | 任务管理 |
| 9 | `meetings` | 20 | 会议记录 |
| 10 | `files` | 31 | 文件存储（含版本链 + 保密级别 + 附件来源追溯 + M5 用途分类与附件链） |
...
| 35 | ~~`scheduler_jobs`~~ | 16 | ~~已退役（2026-09-22 整体移除）~~ 系统调度作业（替代 plan_task_templates，ONCE/CRON/INTERVAL 三种调度类型 + Handler 插件式注册） |
| 36 | ~~`scheduler_executions`~~ | 8 | ~~已退役（2026-09-22 整体移除）~~ 调度执行记录（替代 plan_task_instances + plan_task_logs，PENDING/RUNNING/SUCCESS/FAILED） |
| 37 | `tool_registry` | 10 | 工具注册表（统一记录所有注册到系统的工具 API 元数据，含 exposure_mode 分级控制 SOP-999 直调暴露面） |
| 38 | `business_event_logs` | 18 | 业务事件留痕主表（归因四元组：actor + source/channel/channel_account + action/target + result） |
| 39 | ~~`scheduler_job_logs`~~ | 13 | ~~已退役（2026-09-22 整体移除）~~ 调度器作业日志（含 source/actor，与业务事件同一归因模型） |

> 表清单为 **39 表**：本次仅登记留痕治理涉及的 2 张表；`models.py` 实际为 53 表口径，
> 其余缺失表（如 `node_events`、`rag_retrieval_logs` 等）的补录另案进行。
...
| `source` | String(20) | ✅ | `""` | index（`idx_msg_source_created`） | 流量性质暗记 `user`/`ops`/`auto`/`test`（取代 `console_chat_` 前缀判定；前缀保留作兼容） |
...
| `confirmed_by` | String | ✅ | — | FK→users.id | 认证人（确认录入的用户，溯源到人） |
...
| `confidentiality` | Integer | ✅ | `0` | — | 0公开/1内部/2机密（白名单制） |
| `creator_id` | String | ✅ | — | — | 创建者 |
| `updated_at` | String | ✅ | `_utc_now()` | — | — |
| `is_deleted` | Boolean | ✅ | `False` | — | 软删除 |
| `source_attachment_id` | String | ✅ | — | FK→message_attachments.id | 附件来源追溯 |
| `purpose` | String(50) | ✅ | `"RECORD"` | — | 文件用途分类：EVIDENCE / RECORD / DESIGN / REFERENCE / CHAT |
| `purpose_confirmed` | Boolean | ✅ | `False` | — | 用途是否经人工确认（True=手动，False=自动推断） |
| `attachment_of` | String | ✅ | — | FK→files.id | 主文件 ID（NULL=独立主文件，非NULL=附属文件） |
| `rag_indexed` | Boolean | ✅ | `False` | — | 是否已写入 RAG 向量索引 |
| `rag_collection` | String(100) | ✅ | `""` | — | 所属 RAG collection 名称（""=未索引） |

**FilePurpose 枚举类**：

```python
class FilePurpose(str, Enum):
    EVIDENCE  = "EVIDENCE"    # 证据文件——过程留痕，不可篡改
    RECORD    = "RECORD"      # 记录文件——业务输出正式文档（默认）
    DESIGN    = "DESIGN"      # 设计文件——可迭代工程图纸/模型
    REFERENCE = "REFERENCE"   # 参考资料——辅助决策的规范/标准/案例
    CHAT      = "CHAT"        # 聊天文件——群聊/私聊临时性文件
```
...
#### `evolution_llm_interaction_logs` — 进化 LLM 交互日志
...
| `user_id` | String | ✅ | `""` | — | — |
| `call_category` | String(30) | ✅ | `""` | — | intent/planning/execution/guardian/compression 等 |
| `call_sequence` | Integer | ✅ | `0` | — | 调用序号 |
| `model` | String(100) | ✅ | `""` | — | 模型名 |
| `message_count` | Integer | ✅ | `0` | — | — |
| `tool_count` | Integer | ✅ | `0` | — | — |
| `json_mode` | Boolean | ✅ | `False` | — | — |
| `response_type` | String(20) | ✅ | `""` | — | text/json/tool_call |
| `response_summary` | String(500) | ✅ | `""` | — | 响应摘要（≤500 字） |
| `response_full` | Text | ✅ | `""` | — | 完整响应（截断到 5000 字） |
| `reasoning_content` | Text | ✅ | `""` | — | R1 类模型思维链（deepseek-chat 为空） |
| `finish_reason` | String(50) | ✅ | `""` | — | — |
| `prompt_tokens` | Integer | ✅ | `0` | — | — |
| `completion_tokens` | Integer | ✅ | `0` | — | — |
| `total_tokens` | Integer | ✅ | `0` | — | — |
| `latency_ms` | Integer | ✅ | `0` | — | — |
| `is_error` | Boolean | ✅ | `False` | — | — |
| `error_summary` | String(500) | ✅ | `""` | — | — |
| `created_at` | String | ✅ | `_utc_now()` | — | — |
...
erDiagram
    ...
    scheduler_jobs ||--o{ scheduler_executions : "~~已退役~~"
```

### 3.11 工具注册表模块

#### `tool_registry` — 工具 API 注册表

统一记录所有注册到系统的工具 API 元数据，供 SOP-999 派生白名单、一致性检查（V13/V14/V15）和权限分级。

| 字段 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|------|------|------|--------|------|------|
| `id` | String | 否 | — | PK | API 唯一标识，如 `search_files` |
| `signature` | Text | 否 | "{}" | — | API 签名（参数+返回值 JSON Schema） |
| `display_name` | String(200) | 否 | — | — | 功能一句话说明 |
| `category` | String(20) | 否 | "base" | — | base / business / project |
| `permission_flag` | String(50) | 否 | "all" | — | all（只读+通用）/ write（写入）/ admin（管理） |
| `exposure_mode` | String(20) | 否 | "meta" | — | **meta**=可被 SOP-999 直调 / **sop_only**=必须走专属 SOP。默认值按 permission_flag 分级：all→meta，write/admin→sop_only。破坏性工具（delete/discard/return/unlink）显式标 sop_only |
| `handler_module` | String(200) | 否 | "" | — | 工具 handler 模块路径 |
| `is_active` | Boolean | 否 | true | — | 是否启用 |
| `registered_at` | String(50) | 否 | — | — | 注册时间 |
| `updated_at` | String(50) | 否 | — | — | 更新时间 |

> **自动种子**：容器启动时 `_ensure_tool_registry_seed()` 从 `TOOL_META_MAP` 同步到 `tool_registry` 表，确保 DB 与代码一致。`exposure_mode` 列通过 `_PENDING_COLUMNS` 自动补齐。
...
4. **PostgreSQL advisory lock 是 session 级** —— `SchedulerEngine._tick()` 使用 advisory lock 防并发，但锁在 session 关闭时自动释放。使用 `get_session_raw()` 手动管理 session 生命周期。（`SchedulerEngine` 已随调度器模块于 2026-09-22 整体退役）
...
8. **全景节点（两层制）** —— 旧 `sm_*` 模块（10 张表）已废弃，ORM 模型已移除，旧表待执行 DROP 清理。当前采用 `project_nodes` / `node_dependencies` / `node_deliverables` / `node_events` 四表 Schema。节点类型收敛为两层：`MILESTONE`（里程碑，有子节点的对外可见重大节点）/ `TASK`（任务，叶子节点，携带可计量成果：成果名 + 目标量 + 量纲）；原三层制的"工作包"类型已退场（存量迁移见 `scripts/migrate_node_type_two_layers.py`）。
...
10. **PlanTask 4 张表已废弃** —— ORM 类已删除，DB 表待执行 `005_cleanup_plan_task.sql` 清理。替代者：`scheduler_jobs` + `scheduler_executions`。（替代者亦已于 2026-09-22 随系统调度器模块整体退役）
11. **`project_nodes` 节点层级与进度字段（2026-08-12 补齐）** —— `progress`（VARCHAR，存量只读字段：系统已不再计算写入，节点状态改由 task 的成果完成量驱动）、`parent_node_id`、`child_weight`（保留列，当前不启用）三列后加，由 `_ensure_columns` 自动 ALTER 补齐。同时新增 `uq_pn_node_id` UNIQUE 约束（`node_id` 业务主键唯一，配合 `ProjectNodeRepo.create` 幂等检查防重复创建）。`NodeSnapshot` 同步加 `progress` 字段供状态机引擎读取。父子嵌套深度上限 3 层（`MAX_PARENT_DEPTH`），单父子节点上限 100（`MAX_CHILDREN_PER_PARENT`），`mount_child` 做双向循环检测。
12. **部门维度移除 + 签认字段（2026-09-11）** —— 权限归口收敛为"单位性质 + 等级"，部门不再参与权限判定；`ProjectNode.owner_dept_id` 已废弃（保留列恒为空，默认值 `''`，存量由 `scripts/migrate_remove_department.py` 清空）；`approver_id`/`approved_at` 已废弃（审批→签认）。新增签认三列 `acknowledged_by`（签认人，空=未签认）/`acknowledged_at`（签认时间）/`acknowledged_level`（签认时等级），由 `_ensure_columns` 自动补列。节点状态机收敛为三态（`CONDITIONS_NOT_MET`/`IN_PROGRESS`/`COMPLETED`），`NOT_ACTIVATED` 阻断态退场。`company_info.department`/`permission_groups.department`/`sop_business_flows.require_department_match`+`allowed_departments` 由同一迁移脚本清空/关闭。
13. **签认字段废弃 + 节点判定口径自证化（2026-09-21）** —— `ProjectNode.acknowledged_by`/`acknowledged_at`/`acknowledged_level` 三列**废弃**（保留列与默认值兼容存量，新写入路径不再使用，同 `approver_id`/`approved_at` 处理）。节点是否完结改由**该节点自身的必需成果**单一决定：里程碑不再由子节点三态聚合，`children` 不再进入判定；节点类型改由**声明**决定（取消"有子节点即自动升格为里程碑"）。另：`node_type` 层级约束新增"里程碑不得挂在任务之下"。`SignoffService`、`node_events.node_acknowledged` 事件类型与相关工具/端点/CLI 同批退役。
15. **容器节点与计划启动时间（2026-09-23，全景节点 US-15/16/18）** —— `project_nodes` 新增两列（由 `_ensure_columns` 自动 ALTER 补齐）：
    · `node_role`（VARCHAR(30)，默认 `BUSINESS`）：**容器识别的唯一判定键**——`BUSINESS`（业务节点）/ `TEMP_MILESTONE`（根图下临时里程碑层，项目唯一）/ `TEMP_TASK`（临时任务，待认领）/ `UNCLASSIFIED_SINK`（未归类收容节点，项目唯一，位于临时里程碑层下）。容器为**承载容器**：不要求必需成果、不计完成度、不产生到期提醒、不得作为依赖目标。认领迁正只改 `node_role` 与 `parent_node_id`，**节点编号恒定**（挂载其上的成果/事件/参与关系随之生效）。
    · `planned_start_at`（VARCHAR(50)，默认 `''`）：计划启动时间（ISO8601），激活条件之一——到点即至少进入 `IN_PROGRESS`（`determine_node_status` 新增 `planned_start_reached` 入参）。
    · 新增两个**部分唯一索引**（`_PENDING_INDEXES` 现支持以 `UNIQUE ` 前缀声明）：`uq_pn_project_temp_milestone` / `uq_pn_project_sink`，保证「每项目唯一的临时里程碑层 / 未归类收容节点」。容器按需**懒创建**（空项目零容器），不设超时。
    · `status` 取值域扩展 `DISABLED`（停用：退出管控、**非终态可恢复**；停用期间不计完成度、不提醒、不可挂新任务，节点仍在图中）。排除口径的唯一判定实现见 `services/node_state_machine.excluded_from_rollup`，消费方（世界书结构层/时间层、项目聚合、晨报、快照、认知漂移、初始化体检、节点台账查询、挂载准入）统一调用。
16. **事件归属口径收口为「真实节点」（2026-09-23，US-15.7/15.9/15.10）** —— `project_events.node_id` 的兜底由历史常量 `UNASSIGNED`（`007_migrate_project_events.sql` 播种的**全局假项目行**，`project_id='UNASSIGNED'`，非项目内可解析）改为**本项目唯一的未归类收容节点**（懒创建的真实节点）。写入侧统一在 `ProjectEventAccumulator.accumulate` 收口（全部 `record_*` 均经此），存量对齐见 `scripts/align_unclassified_events.py`（自带备份/回滚/全量扫描）。事件归位（改挂到具体节点）留痕写入该事件 `payload.reassign_history`（改挂前/后归属 + 操作人），与节点迁正留痕（`node_events`）分列、不合并。
17. **创建期一次性落库 + 幂等收口（2026-09-23，US-11）** —— `CreateNodeCommand` 新增 `participant_user_ids` / `shared_file_ids` / `dependencies` / `planned_start_at` / `node_role` 字段；`NodeService.create_node` 按 **节点 → 成果 → 参与单位 → 参与人 → 共享文件 → 依赖** 顺序落库（依赖必须最后：只能指向已存在成果）。节点创建为强成功，其余五类逐项隔离：失败进 `failed[]`、越权/不存在进 `discarded[]` 并回报。**整体幂等**：`node_id` 已存在则直接跳过，不重复写附属对象（此前会重复追加成果）。
18. **创建节点需携带必需成果（2026-09-21）** —— `CreateNodeCommand` 新增 `deliverables` 声明字段；`NodeService.create_node` 强制至少一条 `is_required=true`（否则拒绝，`error_code=40003`），并在同一次调用内落库成果。批量建树不再单独创建成果（原 Phase 2 移除）。

---

### 3.10 系统调度器模块（替代 PlanTask）— 已退役

> **⚠ 已退役（2026-09-22 整体移除）**：系统调度器模块（`emily_core/scheduler/`）及其 14 个作业处理器已整体删除，ORM 类 `SchedulerJob` / `SchedulerExecution` / `SchedulerJobLog` 已移除。对应 DB 表保留为空表（不做 DROP）。以下详细架构为历史记录，仅供参考。
...
### 3.12 留痕治理（操作留痕治理，2026-09-16）

> 本次仅补录留痕治理涉及的两张日志表 + `messages.source`；本文档表清单仍滞后于
> `models.py`（后者为 53 表口径），其余缺失表的补录另案进行。
> 维度口径见 `issues/操作留痕治理/操作留痕治理_需求基线_V1.md` 与 `..._计划_V1.md`。

#### `business_event_logs` — 业务事件日志（结构化留痕主表）

| 字段 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|------|------|------|--------|------|------|
| `id` | String | PK | `_new_uuid()` | — | — |
| `project_id` | String | ✅ | — | index | — |
| `user_id` | String | ✅ | — | index | 操作人（actor）；取不到时记 `unknown`，不得静默不留痕 |
| `user_name` | String(200) | ✅ | `""` | — | — |
| `event_category` | String(30) | ✅ | `""` | — | event/task/meeting/file/permission/system/knowledge/node |
| `event_action` | String(30) | ✅ | `""` | — | created/confirmed/updated/deleted/... |
...
| **`source`** | String(20) | ✅ | `""` | index（`idx_bel_source_created`） | **流量性质**：`user`（真实用户）/ `ops`（人发起的运维）/ `auto`（系统自动）/ `test`（测试流量） |
| **`channel`** | String(30) | ✅ | `""` | — | 传输方式（QQ/企微/小程序/邮箱）；仅 `source=user` 时必填 |
| **`channel_account`** | String(200) | ✅ | `""` | — | 来源标识：渠道账号 / `cli` / `console` / 作业标识 |
| **`result`** | String(20) | ✅ | `""` | — | 三态：`succeeded` / `failed` / `rejected` |
| **`error_reason`** | String(500) | ✅ | `""` | — | 失败或被拒原因（拒绝码表见 `infrastructure/logging/audit.py`） |
| `created_at` | String | ✅ | `_utc_now()` | — | — |

索引：`idx_bel_category_created`、`idx_bel_project_created`、`idx_bel_source_created`。

#### `scheduler_job_logs` — 调度器作业日志 — 已退役（2026-09-22 整体移除）
...
| `started_at` | String | ✅ | `""` | — | — |
| `completed_at` | String | ✅ | `""` | — | — |
| **`source`** | String(20) | ✅ | `""` | index（`idx_sjl_source_created`） | 流量性质，调度作业恒为 `auto` |
| **`actor`** | String(200) | ✅ | `""` | — | 操作人，调度作业恒为 `系统` |
| `created_at` | String | ✅ | `_utc_now()` | — | — |

#### 补列与索引机制

`_ensure_columns()`（`infrastructure/database/session.py`）除补列外，现同时支持：
- `_PENDING_CONSTRAINTS`：补 UNIQUE 约束；
- **`_PENDING_INDEXES`：补普通索引**（本次新增；此前该机制不覆盖索引，既有库上补列后索引不会自动创建）。

新增列/索引的落地步骤：① `models.py` 加 `Column` / `Index`；② `session.py` 的
`_PENDING_COLUMNS` / `_PENDING_INDEXES` 登记（保证既有库经 ALTER / CREATE INDEX 补齐）。
