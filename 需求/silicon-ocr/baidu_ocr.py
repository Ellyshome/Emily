...
CONFIG_FILE = os.path.join(os.path.dirname(__file__), "baidu_config.json")
...
def ocr_image(image_path):
    ...
    if not api_key:
        raise Exception("未在 baidu_config.json 中设置 API 密钥")
...
def extract_page_number(text):
    """从文本中查找 [页码：X] 标注，优先检查开头部分，提取不到则返回 None"""
    # 先在前 200 个字符中查找（覆盖更宽的范围）
    match = re.search(r"\[页码\s*[：:]\s*(\d+)\]", text[:200])
    if match:
        return int(match.group(1))
    
    # 如果开头没有找到页码标注，尝试从最后一行提取
    # 分割文本为行，去除空行
    lines = [line.strip() for line in text.split('\n') if line.strip()]
    
    if lines:
        last_line = lines[-1]
        # 检查最后一行是否为单独的数字且不大于999
        if last_line.isdigit() and len(last_line) <= 3:
            page_num = int(last_line)
            if page_num <= 999:
                print(f"  -> 从最后一行识别到页码: {page_num}")
                return page_num
    
    print(f"  -> 未识别到页码，文本开头为: {text[:20]}")
    return None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("使用方法:")
        print("python baidu_ocr.py img1.jpg img2.jpg img3.png [extension]")
                    ...
                    if output_extension:
                        try:
                            filepath = generate_filename_from_text(
                                result["text"],
                                output_extension,
                                base_dir=os.path.dirname(result["file"]),
