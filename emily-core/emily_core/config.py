...
⚠️ 格式约定（配置清单页依赖，勿随意改动）：
   本文件每个 Config 字段的「赋值语句」后必须紧跟一条字符串字面量作为字段说明
   （即字段赋值下一行紧贴一条用三引号包裹的说明文本）。
   配置清单页（emily_core/config_inventory.py）在运行时解析本文件源码取得字段说明，
   以避免第二份副本漂移；格式不符时该字段说明降级为空，不阻断页面。
"""
