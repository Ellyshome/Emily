...
# ============================================================
#  全局状态机模块 (State Machine) — emily-core/emily_core/state_machine/
# ============================================================


class SMNode(Base):
    """状态机节点主表 — ~90 个项目标准节点"""
    __tablename__ = "sm_nodes"

    id = Column(String, primary_key=True, default=_new_uuid)
    node_id = Column(String(20), nullable=False, unique=True)

    # ── 基础信息 ──
    node_name = Column(String(500), nullable=False)
    stage_id = Column(Integer, nullable=False)
    parent_section = Column(String(200), default="")
    ...
    viewers = Column(String, default="[]")

    # ── 时间 ──
    planned_start_date = Column(String, default="")
    planned_end_date = Column(String, default="")
    actual_start_date = Column(String, default="")
    actual_end_date = Column(String, default="")

    # ── 运行时数据 ──
    precondition_score = Column(Integer, default=0)
    block_reason = Column(String(1000), default="")
    delay_reason = Column(String(1000), default="")
    risk_level = Column(String(20), default="正常")

    # ── 标记 ──
    is_milestone = Column(Boolean, default=False)
    recurrence_type = Column(String(20), default="SINGLE")
    recurrence_completed_count = Column(Integer, default=0)
    ...
    __table_args__ = (
        Index("idx_sm_nodes_stage", "stage_id"),
        Index("idx_sm_nodes_status", "status"),
        Index("idx_sm_nodes_type", "node_type"),
    )


class SMNodeDependency(Base):
    """节点依赖关系表"""
    __tablename__ = "sm_node_dependencies"

    id = Column(String, primary_key=True, default=_new_uuid)
    from_node_id = Column(String(20), nullable=False)
    to_node_id = Column(String(20), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("from_node_id", "to_node_id", name="uq_sm_dep_from_to"),
        Index("idx_sm_dep_from", "from_node_id"),
        Index("idx_sm_dep_to", "to_node_id"),
    )


class SMNodeDeliverable(Base):
    """节点成果物表"""
    __tablename__ = "sm_node_deliverables"

    id = Column(String, primary_key=True, default=_new_uuid)
    node_id = Column(String(20), nullable=False)
    deliverable_name = Column(String(500), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("node_id", "deliverable_name", name="uq_sm_dlv_node_name"),
        Index("idx_sm_dlv_node", "node_id"),
    )
...
class SMStage(Base):
    ...
    id = Column(String, primary_key=True, default=_new_uuid)
    stage_id = Column(Integer, nullable=False, unique=True)

    stage_name = Column(String(500), nullable=False)
    status = Column(String(20), default="未启动")
    total_nodes = Column(Integer, default=0)
    completed_nodes = Column(Integer, default=0)
    progress = Column(Integer, default=0)

    milestone = Column(Boolean, default=False)
    critical_path = Column(String, default="[]")

    boundary_start = Column(String(1000), default="")
    boundary_end = Column(String(1000), default="")

    created_at = Column(String, default=_utc_now)
    updated_at = Column(String, default=_utc_now, onupdate=_utc_now)


class SMStatusHistory(Base):
    """状态变更历史表（审计用）"""
    __tablename__ = "sm_status_history"
    ...
    __table_args__ = (
        Index("idx_ssh_node_time", "node_id", "created_at"),
    )


class SMAuditLog(Base):
    """操作审计日志表"""
    __tablename__ = "sm_audit_logs"
    ...
    risk_marked = Column(Boolean, default=False)
    client_ip = Column(String(50), default="")

    created_at = Column(String, default=_utc_now)


class SMSimulationResult(Base):
    """推演结果缓存表（阶段四使用，阶段一建表预留）"""
    __tablename__ = "sm_simulation_results"

    id = Column(String, primary_key=True, default=_new_uuid)
    simulation_name = Column(String(500), default="")

    assumptions = Column(String, default="[]")
    affected_nodes = Column(String, default="[]")
    total_days_impact = Column(Integer, default=0)
    critical_path_changed = Column(Boolean, default=False)
    risk_level_change = Column(String(50), default="无变化")
    recommendations = Column(String, default="[]")
    new_statuses = Column(String, default="{}")
