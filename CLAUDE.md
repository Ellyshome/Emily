...
**当前架构一句话**：双容器系统（薄插件 `emily_agent` + 独立 `emily-core` FastAPI 内核），采用 **Session 主线 + WorkItem + LangGraph StateGraph（5 节点含 error_analysis 纠错闭环）** 的消息处理架构。旧 M15 八阶段 WorkOrder 管道已完全移除，旧 `sm_*` 全局状态机模块已清理，PipelineBUS 4 节点总线已被 LangGraph 执行引擎替换（2026-07-28）。**内核机制补全（2026-09-15）**：对话循环机制单份实现于 `kernel/react_kernel.py`、计划子图以节点形态挂入父图、重试/超时改节点级声明、上下文入口改框架官方运行时上下文——形态描述见 §6 约束 9b。全景节点 V2 重构进行中。
...
| 9b | **内核机制形态（2026-09-15 补全后）** | 四项机制升级已落地，**形态描述以此为准**（约束 13 要求同步）：① 计划子图(`session/plan_graph.py`)**以节点形态挂入会话父图**（`plan_build → plan → plan_echo`），状态随父线程落检查点，不再由节点内手工 `ainvoke`；② 对话循环机制**单份实现**于 `kernel/react_kernel.py`（`LoopPorts` 注入差异），会话图与会话/工单两张图各为薄适配层，循环本体内不出现调用方字面量；③ 重试与超时改为**节点级声明**（`kernel/policies.py`，`retry_policy=`/`timeout=`）——执行类与挂起类节点**不挂**重试/超时（避免重复副作用与误杀挂起）；④ 上下文**入口**改为框架官方运行时上下文（`kernel/context.py` 的 `KernelContext` + `context_schema` + `context=`），自研 ContextVar 降级为唯一标注兼容桥 `KernelContext.bind_compat()` |
...
| 10b | **脚本参数 schema（Web 控制台前置条件）** | 脚本要能在 Web 控制台（`http://127.0.0.1:18080/console/`）以表单方式调用，必须在 `scripts_registry.yaml` 的 `params:` 字段声明参数 schema——这是约束 11 在脚本侧的等价物：**没有 schema，调用方就不知道参数类型与取值约束**。字段：`name`/`type`(str\|int\|flag\|enum\|multi)/`label`/`help`/`required`/`choices`/`positional`/`group`(互斥组)/`min`/`max`/`options_source`。`options_source`（`users`/`projects`/`nodes`）表示取值来自真实环境——Web 端经 `/api/v1/scripts/options/{source}` 查真实库渲染下拉，**禁止让使用者手抄 UUID**；`choices` 仅用于静态枚举。带子命令的脚本（如 `manage_nodes`）用 `subcommands:` 声明动作分支（`name`/`label`/`help`/`params`），各分支参数集互相独立，前端先选动作再渲染参数。未声明 `params`/`subcommands` 的脚本 Web 端仍可执行，但不接受任何自定义参数，只按注册表 `run_args` 的默认参数跑（`ScriptManager.run_with_defaults`）。**安全**：`params.build_cli_args()` 白名单只对声明了 schema 的脚本生效，未声明的参数一律拒绝；`writes_db: true` 的脚本未显式二次确认时强制降级为 `check_arg` 预览（子命令脚本无法拼出合法预览命令，直接要求显式确认）。 |
...
| 12 | **功能注册接入（元原则）** | 除核心不可拆分功能与主循环（Session 主循环、LangGraph 执行引擎、权限/鉴权引擎、消息收发、DB 基础设施）外，一切新功能必须以可组合模块形态注册接入主系统，禁止裸文件/裸调用/硬编码接线。标准通道：业务工具 → `tools/registry.py`；Hook → `hook_config.json`；SOP → `emily-data/sops/`；脚本 → `scripts_registry.yaml`；Provider（RAG/Email/语音等）→ 实现抽象基类后经注册点接入（RAG/Email 现有 bootstrap if-else 接线属历史遗留，新 Provider 不得沿用此模式）。**注**：原"调度任务 → `JobHandlerRegistry` + `scheduler_config.json`"通道已随定时任务模块于 2026-09-22 整体退役，**系统当前不存在周期性自动触发机制**；需周期执行的能力应改由外部调度（宿主机 cron / 容器编排调度）调用脚本或 API。**提醒义务**：当新增功能无法归入任一注册通道、或出现"实现文件无人调用 / 注册表挂 stub"迹象时，AI 须停下提醒用户先定义注册点或删除，不得留下孤儿代码。 |
| 13 | **内核冻结的适用范围（元原则）** | ⚠️ **本条为最高优先级架构纪律。** 冻结对象是**业务功能增长**：Session 主循环（V2 重构后为 `session/loop.py`）与 LangGraph 执行引擎等内核文件，**永不因业务功能增长而修改**。功能增长一律落在能力层——新工具/新查询 → `tools/registry.py`；新业务流 → 新 SOP 能力（重能力内部用 LangGraph 实现）；多能力协作 → **封装为新能力**，而非扩展编排器或主循环代码。**腐化判据：任何业务功能的 PR diff 到了内核文件，即视为违反本条**，须回退并把功能改走能力层。内核**自身**的机制与形态升级不属业务功能增长，是独立命题：须以专项需求基线立项、评审通过后实施，实施后同步更新本约束所指向的形态描述（当前形态：会话主循环见 `session/loop.py`）。与约束 12 呼应：主循环属"核心不可拆分功能"，不在注册通道之列——也因此**不是业务功能的挂载点**。来源：`需求/Session主体化与WorkItem能力化/需求基线_V2` §六.1；适用范围重述：项目所有者授权（2026-09-11），依据 `需求/LangGraph编排内核化/需求基线_V1` §七。 |
| 14 | **Web 控制台 = 观察窗口（不得自造功能）** | `emy-console`（前端 `emily-core/static/scripts_tool/`、后端 `api/routes/console_resources.py`）定位为**观察 Emily 的窗口**，不是独立业务系统。其**动作/写库类**能力必须复用 **Emily 既有能力**——业务工具层 `tools/` 或运维脚本层 `scripts/`——经既有注册通道接线（ScriptManager / ToolManager / registry），**同源同能力**；只读展示接口允许直查以呈现状态，但须复用 service/repo 层、不得绕过（如 `text()` 裸 SQL 直查表）；禁止在路由层**复刻**既有逻辑或另写一份业务逻辑。既有能力缺失时，须**先在工具层/脚本层补齐再接线到 console**，禁止 console 先行造功能。 |
...
| `emily-core/emily_core/workitem/langgraph_engine/graph.py` | LangGraph StateGraph 执行引擎（5 节点含 error_analysis 纠错闭环） |
| `emily-core/emily_core/workitem/pipeline/bus.py` | `PipelineBUS`：4 节点执行总线（已废弃，仅历史参考，见 §6 约束 9） |
