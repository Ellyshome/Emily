...
| 13 | **Session 主循环冻结（元原则）** | ⚠️ **本条为最高优先级架构纪律。** Session 主循环（V2 重构后为 `session/loop.py`）是**冻结内核**：只因**对话机制本身**的缺陷或升级而修改，**永不因业务功能增长而修改**。功能增长一律落在能力层——新工具/新查询 → `tools/registry.py`；新业务流 → 新 SOP 能力（重能力内部用 LangGraph 实现）；多能力协作 → **封装为新能力**，而非扩展编排器或主循环代码。Session 侧的合法改动仅限：对话行为语义（确认/挂起/续接）、循环终止与超时机制、上下文管理。**腐化判据：任何业务功能的 PR diff 到了 Session 主循环文件，即视为违反本条**，须回退并把功能改走能力层。与约束 12 呼应：主循环属"核心不可拆分功能"，不在注册通道之列——也因此**不是功能的挂载点**。来源：`需求/Session主体化与WorkItem能力化/需求基线_V2` §六.1。 |
